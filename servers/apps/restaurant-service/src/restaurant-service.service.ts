...
export class RestaurantServiceService {
  ...
  // ─── NATS event handler (called from controller) ──────────────────────────────
  ...
  async activateRestaurant(userId: string) {
    await this.restaurantRepo.update(
      { ownerUserId: userId },
      { status: RestaurantStatus.ACTIVE },
    );
    this.logger.log(`Restaurant activated for userId: ${userId}`);
  }

  // ─── Helpers ──────────────────────────────────────────────────────────────────

  private async getOwnedRestaurant(ownerId: string): Promise<Restaurant> {
    const restaurant = await this.restaurantRepo.findOne({ where: { ownerUserId: ownerId } });
    if (!restaurant) throw new NotFoundException('Restaurant not found for this account');
    return restaurant;
  }

  private async assertMenuOwner(menuId: string, restaurantId: string): Promise<Menu> {
    const menu = await this.menuRepo.findOne({ where: { id: menuId } });
    if (!menu) throw new NotFoundException('Menu not found');
    if (menu.restaurantId !== restaurantId) throw new ForbiddenException('Not your menu');
    return menu;
  }

  private async assertSectionOwner(sectionId: string, restaurantId: string): Promise<MenuSection> {
    ...
    if (!menu || menu.restaurantId !== restaurantId) throw new ForbiddenException('Not your section');
    return section;
  }

  private async assertMealOwner(mealId: string, restaurantId: string): Promise<Meal> {
    const meal = await this.mealRepo.findOne({ where: { id: mealId } });
    if (!meal) throw new NotFoundException('Meal not found');
    if (meal.restaurantId !== restaurantId) throw new ForbiddenException('Not your meal');
    return meal;
  }

  private async assertOptionGroupOwner(groupId: string, restaurantId: string): Promise<MealOptionGroup> {
    const group = await this.optionGroupRepo.findOne({ where: { id: groupId } });
  ...
  async updateProfile(ownerId: string, dto: UpdateProfileDto) {
    ...
    return { data: await this.restaurantRepo.findOne({ where: { id: restaurant.id } }), message: 'Profile updated' };
  }

  async updateSettings(ownerId: string, dto: UpdateSettingsDto) {
    const restaurant = await this.getOwnedRestaurant(ownerId);
    await this.restaurantRepo.update(restaurant.id, { ...dto });
    return { data: await this.restaurantRepo.findOne({ where: { id: restaurant.id } }), message: 'Settings updated' };
  }
  ...
  async toggleOpen(ownerId: string) {
    ...
    return { data: { isOpen: newState }, message: newState ? 'Restaurant is now open' : 'Restaurant is now closed' };
  }
  ...
  async getHours(ownerId: string) {
    const restaurant = await this.getOwnedRestaurant(ownerId);
    const hours = await this.hoursRepo.find({ where: { restaurantId: restaurant.id } });
  ...
  async getPublicRestaurantWithMenu(restaurantId: string) {
    const restaurant = await this.restaurantRepo.findOne({ where: { id: restaurantId, status: RestaurantStatus.ACTIVE } });
    if (!restaurant) throw new NotFoundException('Restaurant not found');

    const menus = await this.menuRepo.find({ where: { restaurantId, isActive: true } });

    const menusWithSections = await Promise.all(
      menus.map(async (menu) => {
        const sections = await this.sectionRepo.find({ where: { menuId: menu.id } });
        const sectionsWithMeals = await Promise.all(
          sections.map(async (section) => {
            const meals = await this.mealRepo.find({ where: { sectionId: section.id, isAvailable: true } });
            const mealsWithOptions = await Promise.all(
              meals.map(async (meal) => {
                const optionGroups = await this.optionGroupRepo.find({ where: { mealId: meal.id } });
                const groupsWithOptions = await Promise.all(
                  optionGroups.map(async (group) => {
                    const options = await this.optionRepo.find({ where: { groupId: group.id, isAvailable: true } });
        ...
        return { ...menu, sections: sectionsWithSections };
      }),
    );

    return { data: { ...restaurant, menus: menusWithSections }, message: 'Restaurant retrieved' };
  }
  ...
  async getMenus(ownerId: string) {
    const restaurant = await this.getOwnedRestaurant(ownerId);
    const menus = await this.menuRepo.find({ where: { restaurantId: restaurant.id } });
  ...
  async updateMenu(ownerId: string, menuId: string, dto: UpdateMenuDto) {
    ...
    return { data: await this.menuRepo.findOne({ where: { id: menu.id } }), message: 'Menu updated' };
  }
  ...
  async updateSection(ownerId: string, sectionId: string, dto: UpdateSectionDto) {
    const restaurant = await this.getOwnedRestaurant(ownerId);
    const section = await this.assertSectionOwner(sectionId, restaurant.id);
    await this.sectionRepo.update(section.id, { ...dto });
    return { data: await this.sectionRepo.findOne({ where: { id: section.id } }), message: 'Section updated' };
  }
  ...
  async updateMeal(ownerId: string, mealId: string, dto: UpdateMealDto) {
    ...
    return { data: await this.mealRepo.findOne({ where: { id: meal.id } }), message: 'Meal updated' };
  }
  ...
  async toggleMealAvailability(ownerId: string, mealId: string) {
    ...
    return { data: { isAvailable: newState }, message: newState ? 'Meal is now available' : 'Meal is now unavailable' };
  }
  ...
  async createOptionGroup(ownerId: string, mealId: string, dto: CreateOptionGroupDto) {
    const restaurant = await this.getOwnedRestaurant(ownerId);
    await this.assertMealOwner(mealId, restaurant.id);
  ...
  async updateOptionGroup(ownerId: string, groupId: string, dto: UpdateOptionGroupDto) {
    const restaurant = await this.getOwnedRestaurant(ownerId);
    const group = await this.assertOptionGroupOwner(groupId, restaurant.id);
    await this.optionGroupRepo.update(group.id, { ...dto });
    return { data: await this.optionGroupRepo.findOne({ where: { id: group.id } }), message: 'Option group updated' };
  }
  ...
  async updateOption(ownerId: string, optionId: string, dto: UpdateOptionDto) {
    ...
    return { data: await this.optionRepo.findOne({ where: { id: option.id } }), message: 'Option updated' };
  }
