...
export class CustomerAuthService {
   ...
   * Diagram: Customer sends (Full_Name, Mobile_No, birthdate) → System validates → sends Code.
   */
  async register(dto: RegisterCustomerDto): Promise<{ userId: string; message: string; otp: string }> {
    ...
    // Validate birthdate – must be a past date and user must be at least 10 years old
    ...
    // Create user in PENDING state with no password yet
    ...
    return {
      ...
      otp, // In production this is sent via SMS, not returned here
    };
  }

  /**
   * Step 2 – verfiyCode(): Customer submits OTP.
   */
  async verifyOtp(dto: VerifyOtpDto): Promise<{ message: string }> {
    const user = await this.findActiveCandidate(dto.userId);
  ...
  /**
   * Step 3 – saveCustomer(): Customer sets password → account becomes ACTIVE.
   */
   ...
   */
  async login(dto: LoginCustomerDto): Promise<{ userId: string; role: string }> {
  ...
  private async findActiveCandidate(userId: string): Promise<User> {
